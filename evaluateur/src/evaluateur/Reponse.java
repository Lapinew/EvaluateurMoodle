...
public abstract class Reponse { //Cette classe permet de rassembler une requete SQL, son resultat et le nom du fichier permettant de créer la BD nécéssaire à l'execution de la requete
	...
	public abstract void compareSynthaxe(String requete);
	
	protected void aLaSuite(String nomTest) {
		Gson gson = new Gson();
		ArrayList<Reponse> reponses = null; //On prepare l'arrayList pour récupérer les données dans le json
	...
	protected void premiereTable(String nomTest) {
		Gson gson = new Gson();
		ArrayList<Reponse> buffer = new ArrayList<Reponse>(); //On crée l'arrayList qui va contenir les réponses
	...
	protected Reponse getReponseFromJSON(String nomTest, int numQuestion) {
		...
		try (FileReader reader = new FileReader(nomTest)) { //On lit le json
			...
			reponses = (ArrayList<Reponse>) gson.fromJson(reader, listType); //Récupère l'array du json
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
